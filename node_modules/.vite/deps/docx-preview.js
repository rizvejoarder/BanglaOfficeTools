    ...
    &:before {\r
        ...
        e2.exports = function(e3) {
          ...
          return t2.toString = function() {
            ...
            var l = {};
            if (a2) for (var o = 0; o < this.length; o++) {
              var i = this[o][0];
              null != i && (l[i] = true);
            }
        ...
        t2.DocumentParser = class {
          ...
          parseBodyElements(e3) {
            var t3 = [];
            for (let r3 of l.default.elements(e3)) switch (r3.localName) {
              case "p":
            ...
            return h.foreach(e3, (e4) => {
              switch (e4.localName) {
                ...
                case "tcPr":
                  ...
                  break;
                case "tblStylePr":
                  for (let r3 of this.parseTableStyle(e4)) t3.styles.push(r3);
                  break;
                case "rsid":
                case "qFormat":
                case "hidden":
          ...
          parseParagraphProperties(e3, t3) {
            this.parseDefaultProperties(e3, t3.cssStyle = {}, null, (e4) => {
              if ((0, s.parseParagraphProperty)(e4, t3, l.default)) return true;
              switch (e4.localName) {
                case "pStyle":
          ...
          parseMathElement(e3) {
            ...
            for (const n2 of l.default.elements(e3)) if (d[n2.localName]) r3.children.push(this.parseMathElement(n2));
            else if ("r" == n2.localName) {
              var s2 = this.parseRun(n2);
              s2.type = a2.DomType.MmlRun, r3.children.push(s2);
            } else n2.localName == t3 && (r3.props = this.parseMathProperies(n2));
            return r3;
          }
          parseMathProperies(e3) {
            const t3 = {};
            for (const r3 of l.default.elements(e3)) switch (r3.localName) {
              case "chr":
          ...
          parseDrawingWrapper(e3) {
            ...
            for (var p2 of l.default.elements(e3)) switch (p2.localName) {
              case "simplePos":
          ...
          parseGraphic(e3) {
            ...
            for (let e4 of l.default.elements(t3)) if ("pic" === e4.localName) return this.parsePicture(e4);
            return null;
          }
          parsePicture(e3) {
            ...
            for (var i2 of (t3.cssStyle.position = "relative", l.default.elements(o2))) switch (i2.localName) {
              case "ext":
          ...
          parseDefaultProperties(e3, r3 = null, a3 = null, s2 = null) {
            return r3 = r3 || {}, h.foreach(e3, (n2) => {
              if (!(null == s2 ? void 0 : s2(n2))) switch (n2.localName) {
                case "jc":
          ...
          parseSpacing(e3, t3) {
            ...
            if (r3 && (t3["margin-top"] = r3), a3 && (t3["margin-bottom"] = a3), null !== s2) switch (n2) {
              case "auto":
        ...
        class h {
          ...
          static colorAttr(e3, t3, r3 = null, a3 = "black") {
            ...
            if (s2) return "auto" == s2 ? a3 : m.includes(s2) ? s2 : `#${s2}`;
        ...
        class f {
          ...
          static valueOfBorder(e3) {
            if ("nil" == l.default.attr(e3, "val")) return "none";
        ...
        Object.defineProperty(t2, "__esModule", { value: true }), t2.parseCoreProps = void 0, t2.parseCoreProps = function(e3, t3) {
          const r2 = {};
          for (let a2 of t3.elements(e3)) switch (a2.localName) {
            case "title":
        ...
        t2.parseBorder = s, t2.parseBorders = function(e3, t3) {
          var r3 = {};
          for (let a3 of t3.elements(e3)) switch (a3.localName) {
            case "left":
              r3.left = s(a3, t3);
              break;
            case "top":
        ...
        Object.defineProperty(t2, "__esModule", { value: true }), t2.parseCommonProperty = t2.convertPercentage = t2.convertBoolean = t2.convertLength = t2.LengthUsage = t2.ns = void 0, t2.ns = { wordml: "http://schemas.openxmlformats.org/wordprocessingml/2006/main", drawingml: "http://schemas.openxmlformats.org/drawingml/2006/main", picture: "http://schemas.openxmlformats.org/drawingml/2006/picture", compatibility: "http://schemas.openxmlformats.org/markup-compatibility/2006", math: "http://schemas.openxmlformats.org/officeDocument/2006/math" }, t2.LengthUsage = { Dxa: { mul: 0.05, unit: "pt" }, Emu: { mul: 1 / 12700, unit: "pt" }, FontSize: { mul: 0.5, unit: "pt" }, Border: { mul: 0.125, unit: "pt" }, Point: { mul: 1, unit: "pt" }, Percent: { mul: 0.02, unit: "%" }, LineHeight: { mul: 1 / 240, unit: "" }, VmlEmu: { mul: 1 / 12700, unit: "" } }, t2.convertLength = function(e3, r2 = t2.LengthUsage.Dxa) {
          ...
          if (e3.namespaceURI != t2.ns.wordml) return false;
          switch (e3.localName) {
            case "color":
        ...
        function o(e3, t3, r3) {
          if (e3.namespaceURI != a2.ns.wordml) return false;
          if ((0, a2.parseCommonProperty)(e3, t3, r3)) return true;
          switch (e3.localName) {
            case "tabs":
              t3.tabs = i(e3, r3);
              break;
        ...
        t2.parseParagraphProperties = function(e3, t3) {
          let r3 = {};
          for (let a3 of t3.elements(e3)) o(a3, r3, t3);
          return r3;
        ...
        t2.parseRunProperties = function(e3, t3) {
          let r3 = {};
          for (let a3 of t3.elements(e3)) s(a3, r3, t3);
          return r3;
        ...
        t2.HtmlRenderer = class {
          ...
          renderTheme(e3, t3) {
            ...
            if (l2) for (let [e4, t4] of Object.entries(l2.colors)) s2[`--docx-${e4}-color`] = `#${t4}`;
            const o2 = this.styleToString(`.${this.className}`, s2);
            t3.appendChild(m(o2));
          }
          renderFontTable(e3, t3) {
            for (let r3 of e3.fonts) for (let e4 of r3.embedFontRefs) this.document.loadFont(e4.id, e4.key).then((a3) => {
          ...
          processStyles(e3) {
            ...
            for (const a3 of e3.filter((e4) => e4.basedOn)) {
              ...
              } else this.options.debug && console.warn(`Can't find base style ${a3.basedOn}`);
            }
            for (let t4 of e3) t4.cssName = this.processStyleName(t4.id);
            return t3;
          }
          ...
          processElement(e3) {
            if (e3.children) for (var t3 of e3.children) t3.parent = e3, t3.type == a2.DomType.Table ? this.processTable(t3) : this.processElement(t3);
          }
          processTable(e3) {
            for (var t3 of e3.children) for (var r3 of t3.children) r3.cssStyle = this.copyStyleProperties(e3.cellStyle, r3.cssStyle, ["border-left", "border-right", "border-top", "border-bottom", "padding-left", "padding-right", "padding-top", "padding-bottom"]), this.processElement(r3);
          }
          copyStyleProperties(e3, t3, r3 = null) {
            if (!e3) return t3;
            for (var a3 of (null == t3 && (t3 = {}), null == r3 && (r3 = Object.getOwnPropertyNames(e3)), r3)) e3.hasOwnProperty(a3) && !t3.hasOwnProperty(a3) && (t3[a3] = e3[a3]);
            return t3;
          }
          createSection(e3, t3) {
          ...
          splitBySection(e3) {
            ...
            let u2 = null;
            for (let e4 = s2.length - 1; e4 >= 0; e4--) null == s2[e4].sectProps ? s2[e4].sectProps = u2 : u2 = s2[e4].sectProps;
            return s2;
          }
          renderWrapper(e3) {
...
`);
          ...
          renderNumbering(e3, t3) {
            ...
            for (var s2 of e3) {
              ...
              } else l2 = this.numFormatToCssValue(s2.format);
          ...
          renderHyperlink(e3) {
            ...
            if (this.renderChildren(e3, t3), this.renderStyleValues(e3.cssStyle, t3), e3.href) t3.href = e3.href;
            else if (e3.id) {
          ...
          renderTab(e3) {
            ...
            if (r3.innerHTML = "&emsp;", this.options.experimental) {
              ...
              var s2 = null === (t3 = function(e4, t4) {
                for (var r4 = e4.parent; null != r4 && r4.type != t4; ) r4 = r4.parent;
                return r4;
          ...
          renderRun(e3) {
            ...
            } else this.renderChildren(e3, t3);
            return t3;
          }
          renderTable(e3) {
          ...
          renderVmlChildElement(e3) {
            ...
            for (let r3 of e3.children) r3.type == a2.DomType.VmlElement ? t3.appendChild(this.renderVmlChildElement(r3)) : t3.appendChild(...(0, s.asArray)(this.renderElement(r3)));
            return t3;
          }
          renderMmlRadical(e3) {
            var t3;
            const r3 = e3.children.find((e4) => e4.type == a2.DomType.MmlBase);
            if (null === (t3 = e3.props) || void 0 === t3 ? void 0 : t3.hideDegree) return u(o, "msqrt", null, this.renderElements([r3]));
          ...
          renderMllList(e3) {
            ...
            for (let r3 of this.renderChildren(e3)) t3.appendChild(u(o, "mtr", null, [u(o, "mtd", null, [r3])]));
            return t3;
          }
          renderStyleValues(e3, t3) {
            for (let r3 in e3) r3.startsWith("$") ? t3.setAttribute(r3.slice(1), e3[r3]) : t3.style[r3] = e3[r3];
          }
          renderClass(e3, t3) {
          ...
          styleToString(e3, t3, r3 = null) {
            let a3 = `${e3} {\r
`;
            for (const e4 in t3) e4.startsWith("$") || (a3 += `  ${e4}: ${t3[e4]};\r
`);
            return r3 && (a3 += r3), a3 + "}\r\n";
          }
          ...
          refreshTabStops() {
            this.options.experimental && (clearTimeout(this.tabsTimeout), this.tabsTimeout = setTimeout(() => {
              ...
              for (let t3 of this.currentTabs) (0, n.updateTabStop)(t3.span, t3.stops, this.defaultTabSize, e3);
            }, 500));
          }
        };
      }, 630: (e2, t2) => {
        ...
        t2.computePixelToPoint = function(e3 = document.body) {
          ...
          let h = p.pos + m;
          if (h < d) for (; h < d && u.length < 50; h += m) u.push(Object.assign(Object.assign({}, r2), { pos: h }));
        ...
        function o(e3, t3) {
          ...
          for (let n2 of t3.elements(e3)) switch (n2.localName) {
            case "start":
        ...
        class s {
          ...
          element(e3, t3) {
            for (let r3 = 0, a3 = e3.childNodes.length; r3 < a3; r3++) {
              ...
              if (1 == a4.nodeType && a4.localName == t3) return a4;
            }
            return null;
          }
          ...
          attr(e3, t3) {
            for (let r3 = 0, a3 = e3.attributes.length; r3 < a3; r3++) {
              ...
              if (a4.localName == t3) return a4.value;
            }
            return null;
          }
        ...
        t2.DmlTheme = r2, t2.parseTheme = function(e3, t3) {
          ...
          for (let e4 of t3.elements(l)) switch (e4.localName) {
            case "clrScheme":
        ...
        Object.defineProperty(t2, "__esModule", { value: true }), t2.asArray = t2.formatCssRules = t2.parseCssRules = t2.mergeDeep = t2.isString = t2.isObject = t2.blobToBase64 = t2.keyBy = t2.resolvePath = t2.splitPath = t2.escapeClassName = void 0, t2.escapeClassName = function(e3) {
          ...
          var s;
          if (!a2.length) return t3;
          const n = a2.shift();
          if (r2(t3) && r2(n)) for (const a3 in n) r2(n[a3]) ? e3(null !== (s = t3[a3]) && void 0 !== s ? s : t3[a3] = {}, n[a3]) : t3[a3] = n[a3];
          return e3(t3, ...a2);
        ...
        t2.VmlElement = l, t2.parseVmlElement = function e3(t3, r3) {
          ...
          for (const e4 of n.default.attrs(t3)) switch (e4.localName) {
            case "style":
          ...
          for (const s2 of n.default.elements(t3)) switch (s2.localName) {
            case "stroke":
        ...
        class y {
          ...
          async loadRelationshipPart(e3, t3) {
            var r3;
            if (this.partsMap[e3]) return this.partsMap[e3];
            if (!this._package.get(e3)) return null;
            let n2 = null;
            switch (t3) {
            ...
            if (null == n2) return Promise.resolve(null);
        ...
        function v(e3, t3) {
          ...
          for (let e4 = 0; e4 < 16; e4++) a3[16 - e4 - 1] = parseInt(r3.substr(2 * e4, 2), 16);
          for (let t4 = 0; t4 < 32; t4++) e3[t4] = e3[t4] ^ a3[t4 % 16];
          return e3;
        }
